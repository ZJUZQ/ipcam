...
#define CFG_CMD_COURSE_CHANNELBIND          "CourseChannelBind"         // 录播主机通道绑定关系配置(对应结构体 CFG_COURSE_CHANNELBIND_INFO)
...
// 事件类型 EVENT_IVS_HEATMAP(热度图)对应规则配置
...
typedef struct tagCFG_TAST_SOURCES
{
    ...
    BOOL                            abStartTime;                                // FALSE表示源为实时码流, stuStartTime字段无效; TRUE表示源为录像文件, stuStartTime字段有效
    CFG_NET_TIME                    stuStartTime;                               // 当abStartTime为TRUE时有效，表示源不是实时监视码流, 而是录像文件，该时间表示要分析的录像的开始时间
}CFG_TAST_SOURCES;
...
struct AV_CFG_RecordMode
{
    ...
    AV_int32			nModeExtra1;					// 辅码流录像模式, 0-自动录像，1-手动录像，2-关闭录像
};
...
typedef struct tagCFG_AUDIO_MATRIX_SILENCE
{
	...
	CFG_AUDIO_SILENCE_INPUT_CHN *pstSilenceInputChn; 	// 各个音频输出通道对应的静音输入通道，用户分配
...
typedef struct tagCFG_CALIBRATE_MATRIX_EX_INFO 
{
	...
	CFG_CALIBRATE_MATRIX_ELEMENT	*pstCaliMatrixElement;		// 主从跟踪器标定矩阵配置每个元素值，由用户申请内存
...
typedef struct tagCFG_VTO_BASIC_INFO
{
	 ...
	 NET_CFG_VTO_TYPE emCfgVtoType;						// 设备类型: 1 别墅机 2 门口机 3 梯控机
...
// VTH中远程IPC配置
#define MAX_REMOTE_IPC_NUM 32    // 最大可配置的远程IPC个数
...
typedef struct tagCFG_WLAN_INFO
{
    ...
    int                     nEncryption;                        // 加密模式, 0: off, 1: on, 2: WEP-OPEN, 3: WEP-SHARED, 4: WPA-TKIP, 5: WPA-PSK-TKIP, 6: WPA2-TKIP, 7: WPA2-PSK-TKIP, 8: WPA-AES, 9: WPA-PSK-AES, 10: WPA2-AES, 11: WPA2-PSK-AES
...
// 录播主机物理通道号绑定关系配置
typedef struct tagCFG_COURSE_CHANNELBIND_INFO
{
    int                             nPPT;                           // PPT绑定的物理通道号，-1表示没有绑定
    int                             nBlackboard;                    // 黑板绑定的物理通道号，-1表示没有绑定
    int                             nStudentFeature;                // 学生特写绑定的物理通道号，-1表示没有绑定
    int                             nStudentFullview;               // 学生全景绑定的物理通道号，-1表示没有绑定
    int                             nTeacherFeature;                // 教师特写绑定的物理通道号，-1表示没有绑定
    int                             nTeacherFullview;               // 教师全景绑定的物理通道号，-1表示没有绑定
    int                             nTeacherDetect;                 // 教师检测绑定的物理通道号，-1表示没有绑定
    int                             nBlackboardDetect;              // 板书检测绑定的物理通道号，-1表示没有绑定
}CFG_COURSE_CHANNELBIND_INFO;	
