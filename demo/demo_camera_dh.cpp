...
{
    printf("Usage::");
    printf("\t./demo_camera_dh [Paras] camera_IP\n");
    printf("Paras::\n");
...
int main(int argc, char **argv)
{
    ...
    if (isSave)
    {
        cv::Mat tmpF = camera->getImage(ipc);
    ...
    int64_t frame_num = 0;
    cv::namedWindow("camera_dh", 0);
    while (true)
    {
        frame_num += 1;
        beg = timeStamp();
        //get camera 
        oriF = camera->getImage(ipc);
        if (isUp)
            saveF = oriF;
        else
